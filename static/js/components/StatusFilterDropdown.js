// StatusFilterDropdown.js - Reusable status filter dropdown component
...
Vue.component('status-filter-dropdown', {
    template: `
        <div class="form-floating mb-3">
            <div class="dropdown">
                <button class="btn btn-outline-secondary dropdown-toggle w-100 position-relative" 
                        ...
                        aria-expanded="false">
                        <span class="status-count">Wybierz statusy ({{selectedIds.length}}/{{statuses.length}})</span>
                        <span class="chevron-position"></span>
                </button>
                <div class="dropdown-menu w-100 p-3" aria-labelledby="statusFilterDropdown">
                    <div class="mb-2">
                    ...
                    <div class="overflow-auto" style="max-height: 300px;">
                        <div class="form-check" v-for="status in statuses">
                            <input class="form-check-input" type="checkbox" 
                                :id="'status-' + status.value" 
                                :value="status.value" 
                                v-model="selectedIds">
                            <label class="form-check-label" :for="'status-' + status.value">{{ status.text }}</label>
                        </div>
                    </div>
                </div>
            </div>
    ...
    computed: {
        ...
        areAllSelected() {
            return this.selectedIds.length === this.statuses.length;
        }
    },
    methods: {
        selectAll() {
