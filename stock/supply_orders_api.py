...
def _supply_order_entry_to_dict(entry):
    return {
        ...
        'receiving_cost': _decimal_str(entry.receiving_cost),
...
def _parse_optional_receiving_cost(value):
    ...
    try:
        ...
        return None, JsonResponse(
            {'detail': 'Nieprawidłowa wartość pola receiving_cost.'},
            status=400,
            json_dumps_params={'ensure_ascii': False},
        )


def _parse_stock_entries(raw_entries, *, require_ids=False):
    ...
    for entry in raw_entries:
        ...
        if entry_id is not None:
            try:
                ...
                return None, JsonResponse({'detail': 'Invalid stock_entries id'}, status=400)
            if entry_id <= 0:
                return None, JsonResponse({'detail': 'Invalid stock_entries id'}, status=400)
        elif require_ids:
            return None, JsonResponse({'detail': 'stock_entries id is required'}, status=400)
        ...
        receiving_cost, err = _parse_optional_receiving_cost(entry.get('receiving_cost'))
        if err is not None:
            return err
...
def _resolve_invoice(invoice_id, supplier, *, current_supply_order_id=None):
    if invoice_id is None or invoice_id == '':
        return None, None
    if not isinstance(invoice_id, int) or invoice_id <= 0:
        return None, JsonResponse({'detail': 'Invalid invoice_id'}, status=400)

    invoice = get_object_or_404(Invoice, id=invoice_id)
    assigned_elsewhere = invoice.supply_orders.exclude(id=current_supply_order_id).exists()
...
def _validate_invoice_gross(invoice, total_gross):
    if invoice is None:
        return None
...
def _reload_supply_order(supply_order_id):
    return (
        SupplyOrder.objects.select_related('supplier', 'invoice')
        .prefetch_related(
            Prefetch(
...
def api_create_supply_order(request):
    ...
    if invoice_id is not None and invoice_id != '' and not isinstance(invoice_id, int):
        return JsonResponse({'detail': 'Invalid invoice_id'}, status=400)

    invoice, invoice_err = _resolve_invoice(invoice_id, supplier)
    ...
    gross_err = _validate_invoice_gross(invoice, total_gross)
    if gross_err is not None:
        return gross_err
    ...
    supply_order = _reload_supply_order(supply_order.id)
    return JsonResponse(
...
@require_POST
@require_authenticated_staff_or_superuser
def api_update_supply_order(request, supply_order_id):
    supply_order = get_object_or_404(
        SupplyOrder.objects.prefetch_related('stock_entries'),
        id=supply_order_id,
    )
    ...
    invoice, invoice_err = _resolve_invoice(
        invoice_id, supplier, current_supply_order_id=supply_order.id
    )
    if invoice_err is not None:
        return invoice_err
    ...
    existing_entries = {e.id: e for e in supply_order.stock_entries.all()}
    payload_entry_ids = {e['id'] for e in parsed_entries if e['id'] is not None}

    unknown_ids = payload_entry_ids - set(existing_entries)
    if unknown_ids:
        return JsonResponse(
            {'detail': f'Unknown stock_entries ids: {sorted(unknown_ids)}'},
            status=400,
        )

    to_delete_ids = set(existing_entries) - payload_entry_ids
    for entry_id in to_delete_ids:
        entry = existing_entries[entry_id]
        if entry.reductions.exists():
            return JsonResponse(
                {
                    'detail': (
                        f'Nie można usunąć pozycji magazynowej #{entry_id} — '
                        'ma powiązane wydania.'
                    )
                },
                status=400,
                json_dumps_params={'ensure_ascii': False},
    ...
    for entry_data in parsed_entries:
        if entry_data['id'] is None:
            continue
        stock_entry = existing_entries[entry_data['id']]
        if (
            stock_entry.product_id != entry_data['product_id']
            and stock_entry.reductions.exists()
        ):
            return JsonResponse(
                {
                    'detail': (
                        f'Nie można zmienić produktu pozycji #{stock_entry.id} — '
                        'ma powiązane wydania.'
                    )
                },
                status=400,
        ...
        issued_qty = stock_entry.quantity - stock_entry.remaining_quantity
        if entry_data['quantity'] < issued_qty:
            return JsonResponse(
                {
                    'detail': (
                        f'Ilość pozycji #{stock_entry.id} nie może być mniejsza '
                        f'niż już wydana ({issued_qty} szt.).'
                    )
                },
                status=400,
                json_dumps_params={'ensure_ascii': False},
    ...
    try:
        with transaction.atomic():
            supply_order.supplier = supplier
            supply_order.invoice = invoice
            supply_order.save(update_fields=['supplier', 'invoice'])

            for entry_data in parsed_entries:
                product = Product.objects.get(id=entry_data['product_id'])
                if entry_data['id'] is not None:
                    stock_entry = existing_entries[entry_data['id']]
                    issued_qty = stock_entry.quantity - stock_entry.remaining_quantity
                    stock_entry.product = product
                    stock_entry.quantity = entry_data['quantity']
                    stock_entry.remaining_quantity = entry_data['quantity'] - issued_qty
                    stock_entry.net_cost = entry_data['net_cost']
                    stock_entry.gross_cost = entry_data['gross_cost']
                    stock_entry.stock_type = entry_data['stock_type']
                    stock_entry.vat_rate = entry_data['vat_rate']
                    stock_entry.receiving_cost = entry_data['receiving_cost']
                    stock_entry.save()
                else:
                    StockEntry.objects.create(
                        product=product,
                        ...
                        net_cost=entry_data['net_cost'],
                        gross_cost=entry_data['gross_cost'],
                        stock_type=entry_data['stock_type'],
                        vat_rate=entry_data['vat_rate'],
                        receiving_cost=entry_data['receiving_cost'],
                    )

            if to_delete_ids:
                StockEntry.objects.filter(id__in=to_delete_ids).delete()
    except Exception as exc:
        return JsonResponse({'detail': str(exc)}, status=400)
